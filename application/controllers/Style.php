...
//面料型号管理
	public function list_style($page=1){
		$data = $this->style_model->list_style($page);
		$base_url = "/style/list_style/";
		$pager = $this->pagination->getPageLink($base_url, $data['total'], $data['limit']);
		$this->assign('pager', $pager);
		$this->assign('data', $data);
		$this->assign('page', $page);
		$this->show('style/list_style');
	}

	public function get_style($master_id){
		$list = $this->style_model->get_style($master_id);
	...
	public function use_style($id){
		echo $this->style_model->use_style($id);
	}

	public function save_style(){
		echo $this->style_model->save_style();
	}
//面料入库单
